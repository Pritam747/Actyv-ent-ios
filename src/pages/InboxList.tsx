...
import { fetchApproverInbox } from '../services/InboxList.Service';
...
const InboxList: React.FC = () => {
	...
	const fetchData = async (isFirstLoading = true) => {
		...
		let obj = {
			...
			assignmentStatus: filterValue,
		};
		setDataList([]);
		let res = await fetchApproverInbox(obj);
		if (res.status === Constants.API_SUCCESS) {
			let newItems = res.data?.assignedApplications ?? [];
			setDataList(newItems);
			if (isFirstLoading) setLoading(false);
		} else if (res.status === Constants.API_FAIL) {
			let obj = {
				fileName: 'InboxList.tsx',
	...
	const loadMore = async (event) => {
		...
		if (res.status === Constants.API_SUCCESS) {
			let newItems = [];
			newItems = res.data?.assignedApplications ?? [];
			if (_.size(dataList) === 0) {
				setDataList([...newItems]);
			} else {
	...
	return (
		<IonPage>
			...
			<IonContent fullscreen>
							...
							<IonCol size='10'>
								<IonItem className='pl--0 item-transparent' lines='none'>
									{!toggle && (
										...
										>
											{Constants.APP_INBOX_FILTER.map((filterList) => (
												<IonSelectOption
				...
				{dataList.map((obj) => (
					<IonRouterLink
						routerLink={`/app/inbox-list/inbox-detail/${obj?.applicationId}`}
									...
									<IonCol size='auto' className='pl-0 py-0'>
										<div className='profile-wrap'>
											...
											<h4 className='without-profile fs-22 fw-700'>
												{obj.distributorName.charAt(0).toUpperCase()}
											</h4>
										</div>
									</IonCol>
									<IonCol size='5' className='pr-0 py-0'>
										<h3 className='fs-16 fw-600 mb-5 ellipse'>
											{obj.distributorName}
										</h3>
										<div className='inbox-text-wrap word-breal-all'>
											<p
												className='dark mb-0 fs-11 fw-400'
												id={obj?.applicationId}
											>
												{t('ID')}: <span>{obj.applicationId}</span>{' '}
											</p>

											<p className='dark mb-0 fs-11 fw-400 ellipse'>
												{t('TYPE')}: <span>{obj.type}</span>
											</p>
											<p className='dark mb-0 fs-11 fw-400'>
												{' '}
												{obj?.gstin ? (
													<>
														{t('GSTIN_HEADING')}: <span> {obj.gstin}</span>
													</>
												) : (
													<>
														{t('GSTIN_HEADING')}: <span>{t("NA")}</span>
													</>
												)}
											</p>
											<p className='dark mb-0 fs-11 fw-400'>
													{' '}
													{_.size(obj?.assignees) > 0 ? (
														<>
															{t('ASSIGNEES1')}: <span> {_.join(obj?.assignees,',')}</span>
														</>
													) : (
														<>
															{t('ASSIGNEES1')}: <span>-</span>
														</>
													)}
											</p>
										</div>
									</IonCol>
									<IonCol className='pl-0 pt-0 pr-20 ion-align-self-start ion-text-right'>
										<div className='status-box-wrap p-relative'>
											...
											<p className='absolute-tr fs-10 fw-400 ws-nowrap'>
												{t('LAST_UPDATE1')}: {displayDate(obj.updatedAt)}
											</p>
										</div>
									</IonCol>
									<IonCol
